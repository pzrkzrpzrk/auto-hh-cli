...
| Резюме | `resume list` / `show` / `register` + **🎯 выбор активного резюме** списком из `RESUMES_DIR` |
| Оценить резюме ИИ | `grade` |
| Письмо по id | `cover <vacancyId>` — ввод id и выбор резюме |
| Планировщик | `schedule` — блокирующий, выход по Ctrl+C; резюме берётся из активного |
| Конфигурация | `config` (только просмотр) |
| Сброс данных | `reset` — с подтверждением |

- **Активное резюме** показывается в шапке меню (`Резюме: pavel (pavel.txt)`) и подставляется по умолчанию в шаги `digest`, `cover`, `grade` и `schedule`. Выбор живёт только в текущей сессии меню — на диск ничего не пишется; пункт «Резюме → 🎯 выбрать активное» умеет вернуть «как в .env». В CLI «активного» резюме нет: там работает `-r/--resume` или `.env`.
...
| `auto-hh reset` | Сбросить историю, кэш и дайджесты (файлы + MongoDB) — печатает сводку удалённого |
...
| `--reset` | Перед поиском сбросить историю, кэш и дайджесты — печатает сводку, затем выполняет поиск |
